...
# from https://en.wikipedia.org/wiki/Drug_nomenclature#List_of_stems_and_affixes
...
def match_affixes(token, prefixes, infixes, suffixes):
	for pre in prefixes:
		if re.search("^" + pre + ".+", token):
			return True
	for inf in infixes:
		if re.search(".+" + inf + ".+", token):
			return True
	for suf in suffixes:
...
def extract_entities(token_list):
	...
	for i,token in enumerate(token_list):
		...
		if not appended:
			if match_affixes(token[0].lower(), DRUG_PREFIXES, DRUG_INFIXES, DRUG_SUFFIXES):
