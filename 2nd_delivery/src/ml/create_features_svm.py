...
def extract_features(analysis, entities, e1, e2, s):
    """
    ...
    if analysis.nodes[e1_addr]['lemma'] == analysis.nodes[e2_addr]['lemma']:
        ...
        if len(relation_e1_e2) > 0:
            for elem in e1_e2_path[1:-1]:
                features = features + ["tag_under_e1" + "=" + analysis.nodes[elem]['tag']]
                features = features + ["dep_under_e1" + "=" + analysis.nodes[elem]['rel']]
            features = features + ['dep_under_e1' + "=" + analysis.nodes[e1_e2_path[0]]['rel']]

            e2_path.reverse()
            for elem in e2_path[1:-1]:
                features = features + ["tag_upper_e2" + "=" + analysis.nodes[elem]['tag']]
                features = features + ["dep_upper_e2" + "=" + analysis.nodes[elem]['rel']]

            features = features + ["1under2"]
        # 2under1
        elif len(relation_e2_e1) > 0:
            for elem in e1_e2_path[1:-1]:
                features = features + ["tag_under_e2" + "=" + analysis.nodes[elem]['tag']]
                features = features + ["dep_under_e2" + "=" + analysis.nodes[elem]['rel']]
            features = features + ['dep_under_e2' + "=" + analysis.nodes[e1_e2_path[-1]]['rel']]

            e1_path.reverse()
            for elem in e1_path[1:-1]:
                features = features + ["tag_upper_e1" + "=" + analysis.nodes[elem]['tag']]
                features = features + ["dep_upper_e1" + "=" + analysis.nodes[elem]['rel']]
        ...
        for addr in e1_e2_path[1:-1]:
            if analysis.nodes[addr]['rel'] == 'neg' or \
                    len(analysis.nodes[addr]['deps']['neg']) > 0:
                features = features + ["negativeSent"]
                break
            ...
            for p in pairs:
                ...
                if p.attributes["ddi"].value == "true":
                    try:
                        ...
                        pass  # is_ddi = "false"
                features = extract_features(analysis, entities, id_e1, id_e2, stext)
