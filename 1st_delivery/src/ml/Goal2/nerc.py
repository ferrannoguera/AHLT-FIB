...
def read_corpus():
    ...
    with open('resources/DrugBank.txt') as f:
        corpus = f.read()
    corpus = corpus.split('\n')
    corpus = [sent.split('|') for sent in corpus]
    corpus_words = dict()
    for ent in ['drug', 'drug_n', 'brand', 'group']:
        corpus_words[ent] = []
    print('Reading DrugBank corpus')
    for i in tqdm(range(len(corpus))):
        tokens = tokenize(corpus[i][0])
        for t in tokens:
            if len(t[0]) > 3:
                insort(corpus_words[corpus[i][1]], t[0])
    return corpus_words


CORPUS_WORDS = read_corpus()


def count_sorted(element, sorted_list):
    i = bisect_left(sorted_list, element)
    j = 0
    while i + j < len(sorted_list) and sorted_list[i + j] == element:
        j += 1
    return j
...
def extract_features(s):
    ...
    for i, token in enumerate(s):
        ...
        """
        for ent in ['drug', 'drug_n', 'brand', 'group']:
            l.append("countincorpusas" + ent + "=" + str(count_sorted(token[0], CORPUS_WORDS[ent])))
