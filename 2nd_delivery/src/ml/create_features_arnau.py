...
my_stop_words.remove('or')
my_stop_words.remove('and')
...
def features_lemmas_before_between_after(analysis, e1_addr, e2_addr):
    ...
    for addr in range(1, len(analysis.nodes)):
        if addr == e1_addr or addr == e2_addr:
            ...
            if analysis.nodes[addr]['rel'] != 'punct' and analysis.nodes[addr]['lemma'] in my_stop_words:
                features.append(d[i] + analysis.nodes[addr]['lemma'])
    return features


def int_feature(analysis, path):
    for addr in path:
        if analysis.nodes[addr]['word'] in ['interact', 'interaction']:
            return ['int']
    return []
...
def extract_features(analysis, entities, e1, e2):
    ...
    if analysis.nodes[e1_addr]['lemma'] == analysis.nodes[e2_addr]['lemma']:
        ...
        # Interaction feature
        features += int_feature(analysis, e1_path + e2_path)

        # True if e1 under e2
        relation_e1_e2 = [True for elem in e1_path if elem == e2_addr]
        # True if e2 under e1
        relation_e2_e1 = [True for elem in e2_path if elem == e1_addr]
        # indeplemma => Nomes una entitat el veu
        # dependentlemma => El veuen les dos entitats
        # dep => Dels indep lemmas (S'hauria de diferencia entre e1 i e2
        if len(relation_e1_e2) > 0:
            # features = features + ["1under2"]
            for elem in e1_e2_path[1:-1]:
                # features = features + ["indeplemma" + "=" + analysis.nodes[elem]['lemma']]
                features = features + ["depe1" + "=" + analysis.nodes[elem]['rel']]
            features = features + ['depe1' + "=" + analysis.nodes[e1_e2_path[0]]['rel']]
            # e2_path.reverse()
            for elem in e2_path[1:-1]:
               features = features + ["deplemma" + "=" + analysis.nodes[elem]['lemma']]
            """
            first = True
            for elem in e2_path[:-1]:
                head = analysis.nodes[elem]['head']
                if first and analysis.nodes[head]['tag'][0] == "V":
                    # features = features + ["nearestVerb" + "=" + analysis.nodes[elem]['lemma']]
                    features = features + ["relToNearestVerb" + "=" + analysis.nodes[elem]['rel']]
                    break
            """
        elif len(relation_e2_e1) > 0:
            # features = features + ["2under1"]
            for elem in e1_e2_path[1:-1]:
                # features = features + ["indeplemma" + "=" + analysis.nodes[elem]['lemma']]
                features = features + ["depe2" + "=" + analysis.nodes[elem]['rel']]
            features = features + ['depe2' + "=" + analysis.nodes[e1_e2_path[-1]]['rel']]
            # e1_path.reverse()
            for elem in e1_path[1:-1]:
               features = features + ["deplemma" + "=" + analysis.nodes[elem]['lemma']]
            """
            first = True
            for elem in e1_path[:-1]:
                head = analysis.nodes[elem]['head']
                if first and analysis.nodes[head]['tag'][0] == "V":
                    # features = features + ["nearestVerb" + "=" + analysis.nodes[elem]['lemma']]
                    features = features + ["relToNearestVerb" + "=" + analysis.nodes[elem]['rel']]
        ...
        """
        print(features)
        print("both:", e1_e2_path)
        print("e1:", e1_path)
        print("e2:", e2_path)
        print("e1:", e1, "e2:", e2)
        print("e1_addr:", e1_addr)
        print("e2_addr:", e2_addr)
        print("\n\n")
        """
    return features
